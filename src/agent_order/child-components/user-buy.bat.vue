	...
	export default{
		...
		methods : {
			...
			selectBrandData(brand){
				// console.log(brand);
				console.log(this.report.times);
				if(isNaN(this.report.times)){
					if(this.report.times){
						if(this.report.times="首次"){
							this.tableDatas = [];
							for(let data of this.allPriceDatas[0]){
								if(data.brand == brand){
									this.tableDatas.push(data);
								}
							}
						}
						if(this.report.times="二次"){
							this.tableDatas = [];
							for(let data of this.allPriceDatas[1]){
								if(data.brand == brand){
									this.tableDatas.push(data);
								}
							}
					...
					this.tableDatas = [];
					for(let i=0;i<this.allPriceDatas.length;i++){
						if(this.report.times == this.allPriceDatas[i][0].parent_id){
							for(let j=0;j<this.allPriceDatas[i].length;j++){
								if(this.allPriceDatas[i][j].brand == brand){
									this.tableDatas.push(this.allPriceDatas[i][j]);
								}
							}
						}
					}
	...
	<div>
		...
		<div class="main-warpper">
			...
			<div>
				...
				<div class="text_top" v-if="priceWay ==1">
				...
				<div class="text_top" v-if="priceWay==2 && selectCity && selectArea ">
					...
					<el-button size="large" type="primary" @click="showOrder" class="text_top">生成订单</el-button>
				</div>
			</div>
			<!-- 生成订单的弹出框开始 -->
