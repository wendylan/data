			...
			return {
				...
				pickerOptions: {
					shortcuts: [{
						text: '查询最新报价',
						onClick(picker) {
							ajaxCustom.ajaxGet(window.$vueData.vueHandle,"dingoapi/getLastMarketPrice", (responese)=>{
								console.log(responese);
								let lastMaketData = responese.body.data.parentData;
								picker.$emit('pick', lastMaketData[0].created_at);
							}, (responese)=>{
								alert("发生了意想不到的情况, 请重试.");
								console.log(responese)
							});
						}
					}],
		...
		methods : {
			init(){
				ajaxCustom.ajaxGet(this,"dingoapi/getSteelMarketList", (responese)=>{
					console.log(responese);
					let resData = responese.body.data;
					if(responese.body.status_code ===  200){
						// this.tableData = resData.data.steelMarketList;
						this.userProjectInfo.item = resData.userProjectInfo;
						this.getAllBrands();
					}else{
						alert("暂无数据");
					}
			...
			getMarketDatas(date){
				...
				ajaxCustom.ajaxGet(this,"dingoapi/getMarketDatasAtDate", { params : { date : date } },(responese)=>{
					...
					if(responese.body.data.length){
						...
						window.$vueData.lastReportId = responese.body.data[responese.body.data.length-1].id;
						this.report.item = responese.body.data;
					}else{
						alert("该日期没有数据");
					}
			...
			getMarketDatasFromId(){
				...
				let allBrandName = [];
				for(let item of this.projectInfo.brands){
					allBrandName.push(item.text);
				}
				ajaxCustom.ajaxGet(this, "dingoapi/getInitForSteelDeal", {params : {
					id : this.report.reportTime ,
					brands : allBrandName,
					city : this.projectInfo.city,
					area : this.projectInfo.area,
					address : this.projectInfo.add
				}}, (responese)=>{
					...
					// 计算运费
					this.searchTransportPrice(responeseData.transportDatas);
			...
			searchTransportPrice(transportDatas){
				for(let data of this.tableDatas){
					for(let transportData of transportDatas){
			...
			<el-dialog v-model="isCreateProjectBox" >
			    <el-form>
				    ...
				    <el-form-item label="品牌范围:" :label-width="formSeting.formLabelWidth" >
						<el-select  v-model="allBrand.selected"  placeholder="请选择品牌" style="width:88%;" multiple>
			...
			<div class="header-select">
				<div>
					...
					<div class="project-info" v-if="projectInfo">
						...
						<div>
							<span><b>品牌范围</b> : <span v-for="item of projectInfo.brands">{{ item.text + ' ' }}</span> </span>
						</div>
					</div>
				</div>
				<div>
				...
				<table class="header-table" width="100%" border=1>
					<thead>
						<tr>
							...
							<template v-if="tablecolumnDisplay.orderInput">
								<th rowspan="3">
									<p>输入数量</p>
									<el-button size="mini">生成订单</el-button>
								</th>
							</template>
						</tr>
						<tr>
				...
				<tbody>
					<tr v-for="data in tableDatas">
						<template v-if="data.display">
							...
							<template v-if="tablecolumnDisplay.orderInput">
								<td> <input class="order-input" type="text"/> </td>
							</template>
						</template>
					</tr>
				</tbody>
