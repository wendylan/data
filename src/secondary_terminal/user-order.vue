<script>
	...
	export default{
		...
		created : function(){
			ajaxCustom.ajaxGet(this, "dingoapi/getUserOrder", (response)=>{
		...
		methods: {
			showdata(index){
				this.buyData = this.currentOrderInfo[index];
			...
			// 撤销订单
			revokedata(id){
				ajaxCustom.ajaxPost(this,'/dingoapi/cancelOrder',{'id' : id},(responese)=>{
					console.log(responese);
					for(let data of this.orderInfo){
						if(data.id == id){
							data.status = -1;
			...
			orderUpdate(data){
				for (var i = 0; i < this.orderInfo.length; i++) {
					if(this.orderInfo[i].id == data.id){
						this.orderInfo[i] = data;
						break;
					}
				}
				for (var i = 0; i < this.currentOrderInfo.length; i++) {
					if(this.currentOrderInfo[i].id == data.id){
						this.currentOrderInfo[i] = data;
						break;
					}
				}
				this.buyData = data;
...
<template>
	<div>
		...
		<div class="main-warpper">
			...
			<!-- 钢材采购确认表的弹出框开始 -->
			<el-dialog v-model="showOrder" title="计划单"  size="large" style="text-align:center;">
				<order 
				@exit='exitOrder' 
