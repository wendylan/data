...
export default{
	...
	computed : {
		...
		totalPrice(){
			...
			for(let i = 0;i < order.length; i++){
				sum += parseFloat(
					((order[i].out_warehouse_amount?order[i].out_warehouse_amount:order[i].amount)*((order[i].unit_price|0)+(order[i].freight|0))).toFixed(2)
				);
			}
...
<template>
	<div style="width: 80%;margin: 0 auto;">
			...
			<table>
		        ...
		        <tbody>
		            <tr v-for="(order, index) in orderList">
		                ...
		                <td>{{ transportFunction?((order.warehouse=="钢厂直送")?(order.freight=="-"?"待议":order.freight):0):(order.freight=="-"?"待议":order.freight)  }}</td>
		                <td>
		                ...
		                <td>{{ 
			                makePriceStr(
			                	parseInt(transportFunction?((order.warehouse=="钢厂直送")?(order.unit_price+order.freight):order.unit_price):(order.unit_price+order.freight))
						...
						<template v-for='(item, index) in logistics_info'>
							<el-form :rules="rules" ref="ruleForm" :model="logistics_info[index]" :inline="true" style="margin-bottom:0;">
								<el-form-item label="车号:">
									<el-autocomplete class="inline-input same_width" v-model="item.plate_number" :fetch-suggestions="querySearchCar" placeholder="请输入车号" size="small" @select="handleSelect(item)" style="width: 100px;"></el-autocomplete>
								</el-form-item>
		...
		<div slot="footer" class="dialog-footer" style="clear:both;text-align:center;">
			...
			<el-button type="success" @click="confirmReceive" v-if='orderStatus==4'>确认订单</el-button>
