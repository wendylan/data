...
export default{
	...
	computed : {
		...
		totalOutWareHouseNum(){
			let order = this.orderList;
			let num = 0;
			for(let i = 0; i < order.length;i++){
				if(order[i].out_warehouse_amount){
					num += parseFloat(order[i].out_warehouse_amount);
				}
			}
		...
		totalPrice(){
			...
			for(let i = 0;i < order.length; i++){
				sum += parseFloat(
					((order[i].out_warehouse_amount?order[i].out_warehouse_amount:order[i].amount)*((order[i].price|0)+(order[i].cost_freight|0))).toFixed(2)
				);
			}
	...
	methods : {
		...
		saveOrder(){
			...
			// console.log(this.validateCarInfo());
		...
		seperateBill(index){
			...
			this.orderList[index+1].amount = 0;
		...
		newOrder(order){
			...
			for (var i = 0; i < this.orderList.length; i++) {
				...
				this.$set(olddata[i], 'price', order[i].price);
				this.$set(olddata[i], 'warehouse', order[i].warehouse);
				this.$set(olddata[i], 'price_source', order[i].price_source);
...
<template>
	<div style="width: 80%;margin: 0 auto;">
		<div v-if="orderStatus<3">
			<div v-show="!isSelectNeeding">
				...
				<p style="text-align:left;" v-show="orderStatus !=0">
					...
					<el-steps :space="100" finish-status="success" style="display: inline-block;vertical-align: middle;" align-center>
						...
						<el-step v-if="orderStatus==3" title="已确认" status="finish"></el-step>
						<el-step  v-if="orderStatus==-1 || orderStatus==-2|| orderStatus==-3 " title="已取消" status="error"></el-step>
					</el-steps>
		  		</p>
		  		<div style="float: right; margin-top: -10px;"><span>编号：</span>{{'采'+data.order_number}}</div>
				<table>
			        <thead>
			            <tr>
			                <th>品名</th>
			        ...
			        <tbody>
			            <tr v-for="(order, index) in orderList" :class="rowClass(order)">
			                ...
			                <td>
			                	<el-input size="small" type="number" v-model="order.price" min="1000" max="10000" style="width:70px;" ></el-input>
			                </td>
			                ...
			                <td>
			                	<el-input v-model="order.out_warehouse_amount" type="number" size="small" min="0" style="width:80px;" title="请输入一个确切的数"></el-input>
			                </td>
			                ...
			                <td>
			                	<el-input v-model='order.remark' type="text" size="small" ></el-input>
			                </td>
			                <td>
			            ...
			            <tr>
			            	...
			            	<td colspan="3"></td>
			            	<td>{{ totalNum }}</td>
			            	<td>{{totalOutWareHouseNum}}</td>
			            	<td>{{ totalPrice }}</td>
			            	<td colspan="4"></td>
			            	<td ><el-button @click="showOrderDetail" type="success" size="small" v-if="!isSelectNeeding">订单采购</el-button></td>
			            </tr>
			        </tbody>
			    </table>
			...
			<div v-show="isSelectNeeding" style="margin-top: 20px;">
				<order-sales :data="project" @confirmOrder="newOrder" :old="orderList" :open="isSelectNeeding" @getOpen="openBox" :id="id"></order-sales>
			</div>
		</div>
		...
		<div v-else>
			<div>
				...
				<p style="text-align:left;" v-show="orderStatus !=0">
		  		...
		  		<div style="float: right; margin-top: -10px;"><span>编号：</span>{{'销'+data.order_number}}</div>
				<table>
			        <thead>
			            <tr>
			                <th>品名</th>
		...
		<div slot="footer" class="dialog-footer" v-if="!isSelectNeeding">
			...
			<el-button type="success" @click="sendOrder" v-if="orderStatus==1 || orderStatus ==2" >发给客服</el-button>
		</div>
	</div>
</template>
<style scoped>
