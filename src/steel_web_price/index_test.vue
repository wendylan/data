<script> 
	...
	import { Button, Table, TableColumn, DatePicker,Select,Option } from 'element-ui'
	export default{
		components : {
			headbar,
			titleBox, 
		...
		created(){ 
				...
				this.reportTimesOption.push({'id':this.tableDatas.reportTimes-1,'date':this.tableDatas.date});
				...
				if(date){
					ajaxCustom.ajaxGet(this,"api/getPriceTimesByDate", { params : { date : date } },(responese)=>{
						if(responese.body.data.length){
							...
							this.reportTimes = this.reportTimesOption[responese.length-1].date;
...
<template>
	<div>
		...
		<div class="main-box">
			...
			<div class="search">
				<date-picker :selectDate="selectDate" @getDateData="getMarketDatas" 
				 :reportTimes="reportTimes" :reportOption="reportTimesOption" @showTime ="showMarkData"></date-picker>
			</div>
