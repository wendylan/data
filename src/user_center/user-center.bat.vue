	...
	export default{
		...
		methods:{
			...
			updateProject(index){
				...
				this.isEdit = index;
				this.cache = JSON.parse(JSON.stringify(this.projects[index]));
				this.cache.brands = this.projects[index].brands;
			},
			// 删除项目
			delProject(index){
			...
			saveEdit(index){
				...
				this.projects[index].status = 0;
				this.projects[index].brands = this.cache.brands.toString();
...
<template>
	<div class="main-warpper">
				...
				<tbody>
					<tr v-for="(data,index) in currentProject">
						<template  v-if="isEdit!=index">
							...
							<td>{{ payToText(data.settlement.priceType) }}</td>
							<td>
						...
						<template v-else>
							...
							<!-- 结算方式 -->
							<td>
								<el-select v-model="cache.settlement.priceType" style="width:150px;">
									<el-option v-for="data in projectPayItems" :label="data.text" :value="data.num"></el-option>
								</el-select>
							</td>
							<!-- 结算条件 -->
							<td>
								<el-select v-model="cache.settlement.calculateType" style="width:100px;" size="small">
								...
								<template v-else>
									<label>	网价 </label>
									上浮：
									<el-input v-model="cache.web_price_down" type="number" style="width:70px;" size="small"></el-input>
									<span>元/吨</span>
								</template>

								<br>
								<label style="width:90px;">运费:</label><el-input type="number" style="width:100px;margin-top:10px;" min="0" size="small" v-model="cache.settlement.freight"></el-input>
								<span>元/吨</span>
								<br>
								<label style="width:90px;">过磅费:</label><el-input type="number" style="width:100px;margin-top:10px;" min="0" size="small" v-model="cache.settlement.ponderation_price" ></el-input>
								<span>元/吨</span>
								<br>
							</td>
							<td>
		...
		<!-- 分页 -->
		<div class="block">
			<el-pagination :current-page="currentPage" :page-size="2" layout="total, prev, pager, next, jumper" :total="projects.length" @current-change="changePage">
		    </el-pagination>
		</div>
	</div>
</template>
