			...
			// tab控件
				handleClick(tab) {
					if(tab.label == "近一周订单") {
						...
						var oneweek = new Date(now - 7*24*60*60*1000);
						var nowtime = new Date(now);
						oneweek = this.dateFormat(oneweek);
						nowtime = this.dateFormat(nowtime);
						for(let data of this.orderInfo) {
							var dateFor = data.created_at.slice(0, 10);
							if(dateFor >= oneweek && dateFor <= nowtime){
...
<template>
	<div class="main-warpper">
		...
		<el-tabs v-model="activeName" @tab-click="handleClick" >
		    <el-tab-pane label="近一周订单" name="first"></el-tab-pane>
		    <el-tab-pane label="全部订单" name="second"></el-tab-pane>
		    <el-tab-pane label="处理中" name="third" ></el-tab-pane>
		    <el-tab-pane label="未处理" name="fourth" :status="0"></el-tab-pane>
		    <el-tab-pane label="已完成" name="firth"></el-tab-pane>
		    <el-tab-pane label="已取消" name="sixth"></el-tab-pane>
		</el-tabs>
		...
		<div>
			...
			<el-table ref="multipleTable" :data="currentOrderInfo" border tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
