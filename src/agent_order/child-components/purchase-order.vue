...
<template>
	<div class="main-warpper">
		...
		<el-dialog :visible.sync="dialogOrderVisible" title="钢材采购确认表" style="text-align:center;" size="middle">
			...
			<div style="text-align:left;" v-if="this.buyData.status!=0">
				<label>
					<b>供货信息</b>
				</label>
				<p>
			...
			<div style="text-align:left;margin-top:20px;width:60%;" v-if="this.buyData.status!=0">
			...
			<div slot="footer" class="dialog-footer" style="clear:both;text-align:center;">
				<el-button @click="dialogOrderVisible = false" v-if="this.buyData.status==0">撤销订单</el-button>
				<el-button @click="dialogOrderVisible = false" >返回</el-button>
				<el-button type="primary" @click="saveOrder" v-if="this.buyData.status<0">重发订单</el-button>
			</div>
		</el-dialog>
		<!-- 采购订单的钢材采购确认表的弹出框结束 -->
